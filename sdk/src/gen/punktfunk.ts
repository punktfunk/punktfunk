...
export type ApprovePending = { readonly "expires_in_secs"?: number | null, readonly "grants"?: number | null, readonly "name"?: string | null, readonly "until_disconnect"?: boolean | null }
export const ApprovePending = Schema.Struct({ "expires_in_secs": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Access lifetime from now, seconds. Host stores the absolute deadline.\nAlone: full control until then." })), "grants": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Grant mask. Reserved bits are 400. Omit both access fields to keep\nstored access; `grants` alone is permanent." })), "name": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Label; defaults to the name the device knocked with." })), "until_disconnect": Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Null]).annotate({ "description": "Drop the device's record once its last session ends, rather than at a clock time.\nCombines with `expires_in_secs`: whichever comes first ends the grant. Send it with an\naccess level, never alone (400) — like the other access fields it is part of a whole\nreplacement, so sending `grants` without it clears it." })) }).annotate({ "description": "Approve body. `{}` keeps the knock name and, on re-approve, stored access\n(full/permanent on a first pairing)." })
export type ArmNativePairing = { readonly "expires_in_secs"?: number | null, readonly "fingerprint"?: string | null, readonly "grants"?: number | null, readonly "ttl_secs"?: number | null, readonly "until_disconnect"?: boolean | null }
export const ArmNativePairing = Schema.Struct({ "expires_in_secs": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Access lifetime from now, seconds — not `ttl_secs`. Host stores the\nabsolute deadline. Omit for permanent (`grants` set) or preserved (neither)." })), "fingerprint": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Hex SHA-256 fingerprint that may consume this window. Omit for any\ndevice (trusted-LAN only)." })), "grants": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Grant mask for the device that completes the window. Reserved bits are\n400. Omit both access fields for full (new) or preserved (re-pair)." })), "ttl_secs": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Window length, seconds. Default 120; clamped to 15–600." })), "until_disconnect": Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Null]).annotate({ "description": "Drop the device's record once its last session ends, rather than at a clock time.\nCombines with `expires_in_secs`: whichever comes first ends the grant. Send it with an\naccess level, never alone (400) — like the other access fields it is part of a whole\nreplacement, so sending `grants` without it clears it." })) })
...
export type Grant = { readonly "at": string, readonly "by": string, readonly "forms"?: ReadonlyArray<string>, readonly "path": string, readonly "write": boolean }
export const Grant = Schema.Struct({ "at": Schema.String, "by": Schema.String, "forms": Schema.optionalKey(Schema.Array(Schema.String).annotate({ "description": "The plugin forms that handed this path over (`config`, `game:<entry id>`). A grant\nwith `by: \"form\"` goes when the last of them lets go." })), "path": Schema.String, "write": Schema.Boolean }).annotate({ "description": "One root the operator granted, as recorded on disk. `at`/`by` are audit fields:\nRFC3339 and `console`/`cli`/`legacy`/`form` (a v1 entry has neither)." })
...
export type NativeClient = { readonly "access_level"?: string | null, readonly "expires_unix"?: number | null, readonly "fingerprint": string, readonly "granted_unix"?: number | null, readonly "grants"?: number | null, readonly "name": string, readonly "preferred_pad_slot"?: number, readonly "until_disconnect": boolean }
export const NativeClient = Schema.Struct({ "access_level": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "`full` | `controller` | `view` | `custom`. Derived from `grants`;\nabsent only on hosts older than this field." })), "expires_unix": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()), Schema.Null]).annotate({ "description": "Absolute expiry, unix seconds on the host clock. `null` = permanent.\nAn expired device stays listed; it is just not authorized." })), "fingerprint": Schema.String.annotate({ "description": "Hex SHA-256 of the client certificate — the stable id." }), "granted_unix": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()), Schema.Null]).annotate({ "description": "Last grant time, unix seconds. Display/audit only; never enforced." })), "grants": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "`GRANT_*` bits 0–5. `null` is a pre-grants record, which means full." })), "name": Schema.String, "preferred_pad_slot": Schema.optionalKey(Schema.Number.annotate({ "description": "Player slot this device's pads take, 0-based; `null` = whichever comes free.\nSet through `PUT /session/{id}/player` while the device streams.", "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0))), "until_disconnect": Schema.Boolean.annotate({ "description": "The record is dropped when the device's last session ends, rather than at a clock\ntime. `expires_unix` may also be set; whichever comes first ends the grant." }) })
...
export const NativePairStatus = Schema.Struct({ "armed": Schema.Boolean, "enabled": Schema.Boolean.annotate({ "description": "True when this process started with `--native`." }), "expires_in_secs": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Seconds left in the window. `null` if disarmed, or armed with no expiry (CLI flag)." })), "paired_clients": Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "pin": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])) }).annotate({ "description": "Native pairing window. Unlike GameStream, the host mints the PIN (SPAKE2\nneeds it client-side first); the console displays it." })
...
export type PairedClient = { readonly "fingerprint": string, readonly "label"?: string | null, readonly "not_after_unix"?: number | null, readonly "not_before_unix"?: number | null, readonly "subject"?: string | null }
export const PairedClient = Schema.Struct({ "fingerprint": Schema.String.annotate({ "description": "Lowercase hex SHA-256 of the client certificate DER — the client's stable id here." }), "label": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Operator-assigned display name (`PATCH /clients/{fp}`). The only way to tell two\npaired Moonlight devices apart; absent until somebody names the device." })), "not_after_unix": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()), Schema.Null]).annotate({ "description": "Certificate validity end (unix seconds)." })), "not_before_unix": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()), Schema.Null]).annotate({ "description": "Certificate validity start (unix seconds)." })), "subject": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Certificate subject if the DER parses. Do not display as a device name: every\nmoonlight-common-c client self-signs `CN=NVIDIA GameStream Client`, so this names\nthe protocol. [`Self::label`] is the field to show." })) }).annotate({ "description": "A paired (certificate-pinned) Moonlight client." })
...
export type PendingDevice = { readonly "access_level"?: string | null, readonly "age_secs": number, readonly "expires_unix"?: number | null, readonly "fingerprint": string, readonly "granted_unix"?: number | null, readonly "grants"?: number | null, readonly "id": number, readonly "name": string, readonly "source": string, readonly "until_disconnect": boolean }
...
export const ReleaseDisplayRequest = Schema.Struct({ "slot": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Slot to release (see `state`); omit to release **all** kept displays." })) }).annotate({ "description": "Request body for `releaseDisplay`." })
...
export type ReleaseRequest = { readonly "form": string, readonly "keep": ReadonlyArray<string> }
export const ReleaseRequest = Schema.Struct({ "form": Schema.String.annotate({ "description": "The plugin form letting go, as named when it handed the paths over." }), "keep": Schema.Array(Schema.String).annotate({ "description": "Every path the form still hands over, as saved." }) })
...
export type RunningTitle = { readonly "external_id": string, readonly "pid"?: number | null }
export const RunningTitle = Schema.Struct({ "external_id": Schema.String.annotate({ "description": "Same key the reconcile payload uses." }), "pid": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Pid the provider started, when it knows one. Re-resolved and pinned to start time\nbefore any signal; a stale or recycled pid contributes nothing." })) })
...
export const SourceView = Schema.Struct({ "builtin": Schema.Boolean.annotate({ "description": "Built-in `unom`: not editable, not removable; only its entries may be `verified`." }), "entry_count": Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "error": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])), "fetched_at": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "Unix seconds of the cached index, if any." })), "name": Schema.String, "public_key": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])), "signed": Schema.Boolean.annotate({ "description": "Unsigned sources still serve; the console flags them." }), "stale": Schema.Boolean.annotate({ "description": "Last refresh missed; entries still install — the pin travelled with the entry." }), "url": Schema.String })
...
export const UpdateJobInfo = Schema.Struct({ "received_bytes": Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "stage": Schema.String.annotate({ "description": "`downloading` | `verifying` | `applying` | `restarting`." }), "started_unix": Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "target_version": Schema.String, "total_bytes": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null])) }).annotate({ "description": "In-process apply, or a leftover installer that has not resolved yet." })
...
export type UpdateNativeAccess = { readonly "clear_expiry"?: boolean | null, readonly "expires_in_secs"?: number | null, readonly "grants"?: number | null, readonly "until_disconnect"?: boolean | null }
export const UpdateNativeAccess = Schema.Struct({ "clear_expiry": Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Null]).annotate({ "description": "`true` makes access permanent. Mutually exclusive with `expires_in_secs` (400)." })), "expires_in_secs": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int64" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "New expiry from now, seconds. `0` expires now. Omit to keep current.\nMutually exclusive with `clear_expiry` (400)." })), "grants": Schema.optionalKey(Schema.Union([Schema.Number.annotate({ "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), Schema.Null]).annotate({ "description": "New `GRANT_*` mask; reserved bits are 400. Omit to keep current grants." })), "until_disconnect": Schema.optionalKey(Schema.Union([Schema.Boolean, Schema.Null]).annotate({ "description": "Drop the record once the device's last session ends. Omit to keep the current setting." })) }).annotate({ "description": "Partial PATCH of a paired device's grants/expiry. Omitted fields keep their\ncurrent value." })
...
export type ReleasePluginAccessRequestJson = ReleaseRequest
export const ReleasePluginAccessRequestJson = ReleaseRequest
export type ReleasePluginAccess200 = PluginAccessSnapshot
export const ReleasePluginAccess200 = PluginAccessSnapshot
export type ReleasePluginAccess400 = ApiError
export const ReleasePluginAccess400 = ApiError
export type ReleasePluginAccess401 = ApiError
export const ReleasePluginAccess401 = ApiError
export type ReleasePluginAccess403 = ApiError
export const ReleasePluginAccess403 = ApiError
export type ReleasePluginAccess500 = ApiError
export const ReleasePluginAccess500 = ApiError
...
export const make = (
  ...
  return {
    ...
    "releasePluginAccess": (plugin, options) => HttpClientRequest.post(`/api/v1/plugin-access/${plugin}/release`).pipe(
    HttpClientRequest.bodyJsonUnsafe(options.payload),
    withResponse(options.config)(HttpClientResponse.matchStatus({
      "2xx": decodeSuccess(ReleasePluginAccess200),
      "400": decodeError("ReleasePluginAccess400", ReleasePluginAccess400),
      "401": decodeError("ReleasePluginAccess401", ReleasePluginAccess401),
      "403": decodeError("ReleasePluginAccess403", ReleasePluginAccess403),
      "500": decodeError("ReleasePluginAccess500", ReleasePluginAccess500),
      orElse: unexpectedStatus
    }))
  ),
...
*/
readonly "releasePluginAccess": <Config extends OperationConfig>(plugin: string, options: { readonly payload: typeof ReleasePluginAccessRequestJson.Encoded; readonly config?: Config | undefined }) => Effect.Effect<WithOptionalResponse<typeof ReleasePluginAccess200.Type, Config>, HttpClientError.HttpClientError | SchemaError | PunktfunkError<"ReleasePluginAccess400", typeof ReleasePluginAccess400.Type> | PunktfunkError<"ReleasePluginAccess401", typeof ReleasePluginAccess401.Type> | PunktfunkError<"ReleasePluginAccess403", typeof ReleasePluginAccess403.Type> | PunktfunkError<"ReleasePluginAccess500", typeof ReleasePluginAccess500.Type>>
  /**
