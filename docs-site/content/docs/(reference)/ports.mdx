...
- **The browser client** (preview, off by default) uses UDP 9778, in `punktfunk-native` — see
  [The browser client](/docs/running-as-a-service#the-browser-client-preview). A rule from before
  0.41 lacks it: `sudo ufw app update punktfunk-native && sudo ufw reload`, or `sudo firewall-cmd
  --reload`.
